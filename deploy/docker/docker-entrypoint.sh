...
try_fill_config() {
    ...
    if grep -qE "^[#[:space:]]*$escaped_key\s*=" "$file"; then
        echo_value "$key" "$value"
        if [[ -z "$value" ]]; then
            sed -r "s/^[#[:space:]]*($escaped_key)\s*=\s*(.*)/# \1 = \2/" "$file" > tmpfile && mv tmpfile "$file"
        else
            sed -r "s/^[#[:space:]]*($escaped_key)\s*=\s*(.*)/\1 = $escaped_value/" "$file" > tmpfile && mv tmpfile "$file"
        fi
    # Check if config has a numbering system, but no existing configuration line in file
        ...
        if [[ -n "$value" ]]; then
            ...
            if grep -qE "^[#[:space:]]*$template\s*=" "$file"; then
                echo_value "$key" "$value"
                sed '$a'\\ "$file" > tmpfile && mv tmpfile "$file"
                echo "$key = $value" >> "$file"
            fi
        fi
    fi
...
fill_tuples() {
    ...
    for var in $elements; do
        if grep -qE "\{\s*$var\s*,\s*(true|false)\s*\}\s*\." "$file"; then
            ...
            # backward compatible.
            sed -r "s/($var)\s*\./{\1, true}./1" "$file" > tmpfile && mv tmpfile "$file"
        else
            sed '$a'\\ "$file" > tmpfile && mv tmpfile "$file"
            echo "{$var, true}." >> "$file"
        fi
